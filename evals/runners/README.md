...
python evals/runners/calibrate_judge.py calibrate `
  --labels artifacts/acceptance/<run>/judge-calibration/blind-set-bindings.v2.json `
...
Human rounds must be independent and cover the Blind Set exactly. Every
...
`USER_GATE_REQUIRED` with `aggregation_effect=no_effect`.
