...
`0006_knowledge_document_facts.sql` 增加知识文档、不可变版本、可擦除正文、幂等
Inbox、元数据 Outbox 与索引任务事实源。`0007_pgvector_knowledge_index.sql` 是当前
唯一线性 Head，增加版本化 pgvector 与关键词索引。七个迁移都
具备原子性和可重复执行性；部署时只能将运行时角色授予
仓库之外、已经完成身份认证的工作负载登录角色。

部署必须按 `0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006 -> 0007` 执行正向迁移。`.down.sql`
文件只用于开发环境重置，不会被自动执行。
...
psql "$FLOWPILOT_DATABASE_ADMIN_URL" \
  -v ON_ERROR_STOP=1 \
  -f migrations/0007_pgvector_knowledge_index.sql
```
...
必须严格按 `0007.down -> 0006.down -> 0005.down -> 0004.down -> 0003.down -> 0002.down`
的逆序执行；0007 存在索引、0006 存在知识事实、0005 存在任何 Governance 事实、0004 存在任何
