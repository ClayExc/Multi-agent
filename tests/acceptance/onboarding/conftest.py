"""Shared M5-1 acceptance harness for the onboarding composite graph.

Deterministic fakes for every port the OnboardingCompositeGraph depends on:
the request resolver (multi-round clarification accumulation), the Gateway
probe (simulated device/inventory/permission/ticket MCP with idempotency
cache, readback and failure injection), the execution ledger, the result
artifact store and the approval repository.  Command builders produce
valid TaskCommand digests and security bindings.
"""
...
class FakeOnboardingResolver:
    ...
    def merged_fields(self, task_id: str) -> dict[str, str]:
        # Single-task resolver: every registered reference belongs to the
        # task under test; later submissions accumulate earlier fields.
        merged: dict[str, str] = {}
...
class OnboardingGatewayProbe:
    ...
    async def execute(self, call: OnboardingGatewayCall) -> OnboardingToolResult:
        ...
        # Only VERIFIED outcomes are replayable; UNKNOWN must be reconciled
        # by a fresh read-back (FP-MCP-005) rather than a cached denial.
    ...
    async def _write(self, call: OnboardingGatewayCall) -> OnboardingToolResult:
        ...
        tool = call.action.tool.name
        arguments = call.action.arguments
        if tool == "device.allocate.v1":
            return await self._device_allocate(call, arguments, tool)
        if tool == "permission.grant.v1":
            return await self._permission_grant(call, arguments, tool)
        if tool == "ticket.create.v1":
            return await self._ticket_create(call, arguments, tool)
        return self._failed(call, "RUNTIME_WRITE_TOOL_UNKNOWN")
    ...
    async def _device_allocate(
        ...
        if self.options.device_unknown_once and not self._device_unknown_fired:
            ...
            # The write DID execute upstream (its outcome was lost): count it
            # so the test can prove the graph never executed it a second time.
...
def build_approval_from_card(
    ...
    *,
    ...
    """Build the pending approval record from the FP-APR-001 approval card."""
    basis = card["basis"]
    return Approval(
...
async def build_harness(
    ...
    *,
    ...
    approvals = FakeApprovalRepository()
    config = OnboardingGraphConfig(
        maximum_conversation_rounds=maximum_rounds,
...
async def execute(
    ...
    *,
    run_id: str,
    execution_ref: str = "execution://onboarding/test",
) -> Any:
...
def interrupt_card(harness: OnboardingHarness) -> Mapping[str, Any]:
    """Extract the interrupt card from the last paused graph result."""
