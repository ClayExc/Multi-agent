...
M0～M6 工程候选已经进入主分支。M3～M6 期间并行建设的评测语料、Web 外壳和
Connector 预接入记录见
[`ACCELERATED_DELIVERY_PLAN.md`](./ACCELERATED_DELIVERY_PLAN.md)；该文档保留为
历史来源，不再承担当前派发。现阶段从 M7 开始，先接通真实模型和本地产品，随后
完成两条业务闭环与正式产品执行评测。

> 编号说明：本路线中的 M0～M12 是产品里程碑；历史
> `CHAIN-M1-PLATFORM-01`、`CHAIN-M2-STUDIO-01` 是工程集成增量，二者不
> 表示产品 M1/M2 已完成。自 VPN 切片起，链 ID 使用 `P1` 等产品前缀避免
> 混淆。P1、P2 及 M3～M6 候选均已合入；当前没有激活开发链。
...
当前收口边界：120+36 语料、Hash 冻结文件、Judge 工具和 `make acceptance` 已
合入；产品执行器和人工 Judge 校准尚未完成，移入 M10。M6 的工程收口不构成
整体发布。

## 9. M7：真实模型与本地产品闭环

关联功能：`FP-AGT-001`～`FP-AGT-004`、`FP-UI-001`、`FP-API-001`、
`FP-OBS-001`。

交付：

- 在 Model Gateway 后接入 LiteLLM，首个真实模型使用 DeepSeek V4 Flash。
- 锁定经验证的 LiteLLM 模型标识、超时、重试、预算、计量和错误映射。
- 补齐 `.env.example` 和缺少凭据时的失败关闭提示，不提交真实密钥。
- 装配 Web、FastAPI、Worker、LangGraph、PostgreSQL/Redis 与 VPN 只读 MCP。
- 在 Web、Studio 与 Trace 中暴露可审计节点、模型调用、Interrupt 和恢复状态。

退出条件：

- 中文 VPN 请求可以从 Web 进入真实模型节点并返回带引用回答。
- API、任务时间线、Checkpoint 与 Studio Thread 可按稳定 ID 关联。
- Provider 超时、限流、错误响应和缺少密钥均有稳定错误码与可观察证据。
- 确定性测试不依赖在线模型，真实模型 Smoke 单独分级执行。

## 10. M8：VPN 安全写入产品闭环

关联功能：`FP-MCP-001`～`FP-MCP-006`、`FP-APR-001`、`FP-DATA-002`、
`FP-SEC-001`～`FP-SEC-004`。

交付：

- 将工单写入接到 Web 审批卡、策略判定、MCP Gateway、执行账本和回读结果。
- 提供 Vendor-neutral Ticket Port 与受控 Sandbox/Preview Connector。
- 覆盖参数篡改、审批过期、重复提交、超时后实际成功和 `UNKNOWN` 对账。
- 保持真实企业系统适配为可替换边界，不在本阶段铺满厂商 Connector。

退出条件：

- VPN 排障失败后只创建一张可回读工单。
- 未授权、错租户、摘要错配和过期审批在上游调用前拒绝，逻辑写入数为 0。
- Worker 重启和重复投递不产生重复业务写入。

## 11. M9：新员工复合申请产品闭环

关联功能：`FP-ONB-001`、`FP-APR-001`、`FP-CTX-004`、`FP-OBS-001`。

交付：

- 将设备标准、库存和权限模板三条并行读取接到 API 与 Web 时间线。
- 完成设备与权限两个动作的独立审批、执行、部分失败、恢复和汇总。
- 展示每个动作的依据、影响、状态、回读结果和关联工单。

退出条件：

- AC-E2E-002 可以从 Web 连续走完并由真实产品入口复现。
- 一个动作失败不会回滚已确认的外部成功，也不会重复执行成功动作。
- Handoff、租户隔离、Audit 与 Security Event 维持既有边界。

## 12. M10：产品执行评测与发布候选

关联功能：`FP-EVAL-001`～`FP-EVAL-005`、`FP-OPS-002`～`FP-OPS-004`。

交付：

- 为固定 120+36 Case 注册真实产品执行器并保存实际观察证据。
- 完成 Judge 双轮人工盲测校准、规则评分与 Promotion Gate。
- 在相同数据、模型预算和配置下运行单 Agent/Multi-Agent 三次公平对比。
- 验证 Compose 空卷启动、两个业务演示、安全演示和证据包复算。

退出条件：

- 156 条任务全部进入固定分母，失败、跳过和隔离均不从分母移除。
- P0 安全断言全部通过，无未解释的 skipped Case。
- 每项指标绑定代码、数据集、模型、配置、原始结果和复现命令。
- 满足门禁后再决定是否提升为 `frozen` 或发布候选。
...
| LoRA 过早 | 尚无稳定标签就训练 | 核心发布后再进入 M12 |
