"""Acceptance tests for the M6 incremental-B candidate corpus (goal B1).

The corpus is a candidate-only local dataset (40 functional + 12 safety/fault)
...
- no case-id collisions with the increment-A corpus.
"""
...
from packages.evaluation.incremental_b import (
    CASE_SPECS,
...
def test_candidate_counts_match_incremental_b_quota(cases: list[dict]) -> None:
    ...
    assert sum(sum(values.values()) for values in counts.values()) == 52
...
def test_no_case_id_collision_with_increment_a(cases: list[dict]) -> None:
    incremental_a_ids = {spec.case_id for spec in INCREMENTAL_A_SPECS}
    incremental_b_ids = {case["case_id"] for case in cases}
    assert not (incremental_a_ids & incremental_b_ids)
    assert len(incremental_b_ids) == len(CASE_SPECS)


def test_manifest_hashes_match_committed_files() -> None:
    manifest = load_json_strict(dataset_dir(ROOT) / "manifest.json")
    assert manifest["dataset_id"] == "flowpilot-m6-incremental-b-local"
...
def test_new_fault_profiles_are_registered_in_card() -> None:
    card = (dataset_dir(ROOT) / "dataset-card.yaml").read_text(encoding="utf-8")
    referenced = {
        case["fault_injection"]["profile_id"]
        for case in load_cases(ROOT)
        if case.get("fault_injection")
    }
    for profile_id in referenced:
        assert profile_id in card, f"{profile_id} missing from dataset card"
    for profile_id in (
        "fault.dep.provider-timeout.v1",
        "fault.dep.provider-unknown.v1",
        "fault.dep.gateway-unavailable.v1",
        "fault.dep.partial-success.v1",
        "fault.dep.downstream-timeout.v1",
        "fault.dep.unknown-no-retry.v1",
        "fault.dlp.secret-in-doc.v1",
        "fault.dlp.export-request.v1",
        "fault.dlp.pre-write-scan.v1",
    ):
        path = ROOT / "evals" / "fixtures" / "fault-profiles" / f"{profile_id}.json"
        assert path.is_file(), f"new profile missing: {profile_id}"
