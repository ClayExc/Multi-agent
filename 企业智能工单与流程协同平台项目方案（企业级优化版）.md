...
>
> 文中保留的 VPN 对象、Trace 和数据片段属于早期工程回归 Fixture，用于说明检索、
> 恢复和幂等机制，不再代表正式产品场景或后续里程碑。

> 基于 LangGraph（大模型状态图编排框架）的多智能体企业服务执行平台  
>
> 项目定位：面向企业 IT 服务台，覆盖智能工单、知识库问答、新员工入职、权限变更和审批辅助
>
> 系统职责：补齐信息、查询知识、等待审批并调用受控工具，任务在用户暂离或 Worker 重启后仍可继续
>
...
企业 IT 服务台存在大量跨系统、重复且依赖制度判断的服务请求，例如：

- “这个访问异常需要提交工单，请帮我补齐信息并判断处理组。”
- “研发环境的数据访问规范是什么？请给出制度来源。”
- “给下周入职的新员工申请电脑和系统权限。”
- “把我的报表权限续期三个月，按流程发起审批。”
- “请汇总这条管理员权限申请的变更、风险和审批依据。”
...
> 研发环境的数据访问需要满足哪些条件？请给出当前有效的制度来源。

预期流程：

1. 识别为企业知识查询。
2. 结合可信身份确定租户、岗位和查询目的。
3. 检查问题范围并追问缺失条件。
4. 在检索前执行 ACL、Purpose 和数据分类过滤。
5. 返回带出处、版本和有效期的回答。
6. 允许用户反馈已解决、无帮助或知识过期。
7. 记录反馈结果及所引用知识。

#### 用例 B：知识不足时转为智能工单

在用例 A 中没有足够知识或用户需要人工处理：

1. 汇总问题、业务影响、已确认条件和引用过的知识。
2. 补充影响范围、紧急程度和期望处理时间。
3. 推荐工单分类、优先级和处理组，并展示依据。
4. 生成结构化工单草稿。
...
- 智能工单、知识库问答、新员工入职、权限变更和审批辅助五条本地业务链均能从 Web 完成。
- 至少一个闭环包含人工审批和恢复。
- 至少一个闭环包含并行工具调用。
