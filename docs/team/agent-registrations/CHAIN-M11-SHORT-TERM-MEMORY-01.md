...
READY_NOT_DISPATCHED=memory-security-builder
NOT_YET_ACTIVATED=memory-context-builder,memory-data-builder,memory-runtime-builder,memory-core-composer,memory-quality-builder,memory-integration-verifier
UNSELECTED=none
```

| Agent ID | Role | 主写目标 | 当前状态 | 退出条件 |
|---|---|---|---|---|
| memory-security-builder | S3-PLATFORM | Working Memory 内容安全表面与凭据/DLP 回归 | READY_NOT_DISPATCHED | WP-122 clean Handoff |
| memory-context-builder | S2-RUNTIME | Snapshot、摘要、预算与 Memory Port | DEPENDENCY_WAIT | WP-123 clean Handoff |
