...
2. S1 复核 S6 新 Head 与 Migration/实库证据。
3. S2 `WP-010-a2`：基于已接受的 S6 原语实现 Worker 组合适配器，关闭 004 的消费侧。
4. S1 复核 S2/S6 联合恢复证据。
5. S5 新 Attempt：在完整源码集合上刷新 Workspace 与 `uv.lock`，关闭 005。
6. S7 `WP-040-a1`：从干净控制基线构造临时组合树，运行联合门禁。
7. S1 根据 WP-040-a1 证据决定主分支合入方式和顺序。

消息到达顺序不构成上述顺序；每一步必须由前一步的明确交接解锁。
