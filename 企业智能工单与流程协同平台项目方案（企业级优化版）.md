...
简历同时包含 OpenAI Agents SDK 和 Claude Agent SDK。合理设计不是在一次任务里让两个 SDK 同时管理同一段对话，而是定义统一接口：
...
简历中的“自主进化”容易被理解为生产 Agent 在线修改自己，这在企业系统中风险过高。建议改为“评估驱动的持续优化”：
...
面试项目可用 Keycloak（身份服务）、OPA、Vault 开发模式和 MinIO 演示企业能力；简历中应明确哪些是已实现、哪些是生产化设计，避免把架构方案写成已经落地的结果。
...
提交 5 台显示器采购申请，展示金额规则触发经理审批；在审批中心修改数量并批准，Agent 恢复运行并创建采购申请。
...
> 我设计并实现了流程领航 FlowPilot，一套面向企业 IT 服务台的智能工单 Agent 平台。LangGraph 负责跨节点的持久化业务流程，OpenAI/Claude Agents SDK 通过统一 Runtime Adapter 承载局部专业 Agent，所有业务工具统一经过 MCP Gateway。系统不只完成请求理解、知识与数据查询、人工审批和工具执行，还把 OIDC 身份、租户隔离、RBAC + ABAC、数据脱敏、Prompt Injection 防御、密钥管理和不可篡改审计放在执行链路上，使任务可恢复、可验证、可追责。

### 为什么使用多智能体

> 我先实现了单智能体基线，然后按知识域、上下文和工具权限拆成知识智能体、数据智能体、执行动作智能体和策略智能体。只读查询可以并行执行，而有副作用的工具只交给执行动作智能体，并且必须通过策略节点。这种拆分降低了上下文污染，也便于权限隔离、独立评估和审计。

### 为什么使用 LangGraph

> 这个项目包含追问、并行查询、失败重试、人工审批、暂停恢复和多个结束状态，用普通的推理与行动循环（ReAct）不容易控制。LangGraph 可以把这些状态和条件显式建模，并通过状态检查点组件（Checkpointer）保证任务在等待审批或服务重启后继续执行。

### 为什么同时使用 LangGraph 和 Agents SDK

> 两者不争夺同一层的控制权。LangGraph 是唯一的跨业务流程状态机，管理路由、并行、审批、补偿和最终状态；Agents SDK 只在一个有边界的节点内管理模型工具循环、结构化输出、Guardrail、Handoff 和局部会话。Runtime Adapter 统一不同 SDK 的输入输出，Checkpoint 只保存 SDK Session/Run 的引用，因此不会形成两套互相矛盾的状态。

### Handoff 和多 Agent 如何控制边界

> Handoff 只允许在同一专业能力域内把任务交给受限子 Agent，不能跨越 LangGraph 的审批、策略和执行节点。子 Agent 得到的是裁剪后的上下文和独立工具白名单，不继承调用方全部权限。任何有副作用的工具仍必须经过 MCP Gateway 的实时授权。

### 如何避免智能体死循环

> 我设置了图最大步骤、节点重试上限和任务预算，并对相同 Agent、工具和参数进行重复检测。连续两次状态没有有效变化时直接转人工。业务错误不会盲目重试，只有瞬时系统错误才采用指数退避。

### 如何保证工具调用安全

> MCP 解决工具接入标准化，但不是安全边界。安全控制由 MCP Gateway 强制执行：先用 OIDC 身份构造安全上下文，再通过 RBAC + ABAC 同时校验用户、Agent、用途、数据级别和资源归属；随后做 Schema 校验、参数脱敏、风险审批、短期凭证注入、隔离执行、幂等和回读验证。模型没有直接网络、数据库或密钥权限。

### 如何防御 Prompt Injection

> 我把用户输入、企业文档、网页、附件和 MCP 工具输出都视为不可信数据，并与系统指令分区。系统通过来源标记、内容扫描、检索权限过滤、工具参数约束和输出 DLP 构成多层防线。Prompt 检测模型只能提供风险信号，真正的权限和高风险动作必须由确定性策略与人工审批决定。

### 如何设计 RBAC、数据脱敏和租户隔离

> RBAC 决定角色拥有哪类能力，ABAC 再结合租户、资源归属、数据等级、用途、设备、时间和审批状态做实时决策。所有业务表使用 `tenant_id` 和 PostgreSQL RLS 做数据库兜底；检索前过滤权限，调用模型前最小化和脱敏，工具返回后再次脱敏，界面按字段级权限展示。

### 如何管理密钥

> 密钥只存放在 Vault 或云 KMS 中，应用和 Agent 状态只保存凭证引用。MCP Gateway 通过工作负载身份获取短期令牌并在调用时注入，模型、Prompt、Checkpoint、SDK Session 和日志都看不到明文密钥。轮换、读取失败和异常访问都会进入安全事件。

### Trace 和审计日志有什么区别

> Trace 服务于研发诊断，可以采样并记录节点、模型和工具耗时；审计日志服务于追责和合规，必须追加写、不可随意修改，重点记录谁在什么租户下基于哪版策略访问了什么资源、做了什么动作以及结果。两者通过 Trace ID 关联，但权限、保留期限和存储完全分离。

### 多模态与“自主进化”如何避免成为噱头

> 截图、日志和附件先经过隔离、恶意文件扫描、文本提取、数据分级和 Prompt Injection 检测，Agent 只读取规范化的安全观察结果。所谓自主进化不是生产系统在线改 Prompt 或自我训练，而是把失败轨迹脱敏并人工审核，优先优化 Prompt、路由和工具策略；只有意图分类、字段抽取、工单摘要等窄任务才考虑 PEFT/LoRA，并经过离线评估、灰度和回滚。

### 如何评估

> 我构建了任务级测试集，不只评估回答质量，还测量意图准确率、字段提取、工具选择、工具参数、端到端成功率、循环率、审批拦截率、延迟和成本，并用单智能体作为基线进行消融实验。
...
**流程领航（FlowPilot）｜企业智能工单 Agent 平台**

### 项目描述

个人项目，面向企业 IT 服务台，完成请求理解、知识与数据查询、人工审批、工单执行和结果验证闭环。

核心技术：LangGraph · MCP · OpenAI/Claude Agents SDK · FastAPI · PostgreSQL · OPA · Vault · OpenTelemetry

### 推荐投递版（完成对应实现后使用）

- **企业级有状态 Multi-Agent 编排**：以 LangGraph 作为唯一跨业务流程状态机，建模意图识别、信息补全、计划、并行查询、审批、执行和验证；按编排、知识、数据、执行、策略拆分 Agent，通过 Checkpointer/Interrupt 支持人工介入、断点恢复和失败补偿。
- **Agents SDK 与 MCP 安全执行层**：通过 Agent Runtime Adapter 对接 OpenAI/Claude Agents SDK，在受限节点内实现工具调用、Handoff/子 Agent、Guardrail、会话恢复和 Trace；建设 MCP Gateway 统一接入工单、知识库、数据库和运维工具，强制执行租户隔离、RBAC + ABAC、Agent/工具白名单、审批、幂等和回读验证。
- **企业安全与审计治理**：基于 OIDC 传递用户和工作负载身份，建立数据分级、检索前权限过滤、调用前后 DLP 脱敏及多层 Prompt Injection 防御；密钥由 Vault/KMS 托管并按需注入短期凭证，Trace 与不可篡改审计分流，记录策略版本、决策依据和完整工具执行证据。
- **安全多模态与评估驱动优化**：对文本、截图、日志和附件执行隔离扫描、内容抽取、敏感信息识别和不可信指令检测；以成功率、引用准确率、越权拦截率和人工接管率沉淀脱敏失败轨迹，闭环优化 Prompt、路由和工具策略，并对意图分类、字段抽取、工单摘要等窄任务进行 PEFT/LoRA 离线适配、灰度和回滚。

### 一页简历精简版

- **有状态 Multi-Agent**：LangGraph 编排意图、补全、并行查询、审批、执行与验证，按知识/数据/执行/策略拆分 Agent，并以 Checkpointer/Interrupt 实现人工介入和断点恢复；通过 Runtime Adapter 在局部节点接入 OpenAI/Claude Agents SDK。
- **MCP 与企业安全治理**：以 MCP Gateway 标准化接入工单、知识库、数据库和运维工具，落地 OIDC、租户隔离、RBAC + ABAC、数据脱敏、Prompt Injection 防御、Agent/工具白名单、Vault/KMS 短期凭证和不可篡改审计。
- **多模态与持续优化**：安全解析文本、截图、日志和附件，以任务成功率、引用准确率、越权拦截率和人工接管率沉淀脱敏失败轨迹，迭代 Prompt、路由与工具策略，并对窄任务进行 PEFT/LoRA 离线适配、灰度和回滚。

### 简历真实性边界

- 已完成并有代码、测试或演示证据时使用“实现、构建、落地”。
- 只有架构和接口设计时改为“设计、规划、预留”，不要声称已经接入双 Provider、Vault、OPA 或 LoRA。
- “自主进化”建议改为“评估驱动持续优化”，避免被理解为生产环境在线自改 Prompt、自授予权限或用未审核对话直接训练。
- 面试时准备一张“功能—代码位置—测试用例—指标结果”证据表，确保简历中的每个技术点都能打开代码讲清楚。

量化结果必须在项目实际测试后填写，例如：

```text
- 在 N 条测试任务上，端到端完成率由单 Agent 基线的 A% 提升至 B%。
- 工单字段完整率达到 C%，无效工具调用率降低 D%。
- 并行查询使 P50 任务延迟由 E 秒降低至 F 秒。
- 高风险测试集中，审批策略成功拦截 G/H 个未授权操作。
```
