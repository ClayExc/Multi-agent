"""Acceptance tests for the M6 incremental-C candidate corpus (goal C1).

The corpus is a candidate-only local dataset (16 functional candidates)
validated against the released evaluation-registry without touching the
frozen contracts. Tests cover:

- registry validation of every candidate, both independently and collectively;
- binding completeness (Feature, Fixture, rule assertion, data source) for
  every candidate, and the absence of safety-class/gate tags on functional
  candidates;
...
- no case-id collisions with the increment-A or increment-B corpora.
"""
...
from packages.evaluation.incremental_a import CASE_SPECS as INCREMENTAL_A_SPECS
from packages.evaluation.incremental_b import CASE_SPECS as INCREMENTAL_B_SPECS
from packages.evaluation.incremental_c import (
    CASE_SPECS,
...
def test_candidate_counts_match_incremental_c_quota(cases: list[dict]) -> None:
...
def test_no_case_id_collision_with_increments_a_and_b(cases: list[dict]) -> None:
    incremental_a_ids = {spec.case_id for spec in INCREMENTAL_A_SPECS}
    incremental_b_ids = {spec.case_id for spec in INCREMENTAL_B_SPECS}
    incremental_c_ids = {case["case_id"] for case in cases}
    assert not (incremental_a_ids & incremental_c_ids)
    assert not (incremental_b_ids & incremental_c_ids)
    assert len(incremental_c_ids) == len(CASE_SPECS)
...
def test_approval_candidates_bind_approval_and_write_assertions(
    cases: list[dict],
) -> None:
    for case in cases:
        if case["category"] != "approval_recovery":
            continue
        declared = {
            item["assertion_id"] for item in case["deterministic_assertions"]
        }
        assert "assert.approval.valid.v1" in declared, case["case_id"]
        assert "assert.tool.write_count.v1" in declared, case["case_id"]


def test_long_context_candidates_bind_budget_and_handoff_assertions(
    cases: list[dict],
) -> None:
    for case in cases:
        if case["category"] != "long_context_handoff":
            continue
        declared = {
            item["assertion_id"] for item in case["deterministic_assertions"]
        }
        assert "assert.context.within_budget.v1" in declared, case["case_id"]
        assert "assert.handoff.fields_allowed.v1" in declared, case["case_id"]
...
def test_undecidable_assertion_parameters_are_rejected(
    ...
    broken = copy.deepcopy(
        next(case for case in cases if case["category"] == "approval_recovery")
    )
...
def test_budget_assertion_requires_positive_tokens(
    validator: OfflineRepositoryValidator, cases: list[dict]
) -> None:
    broken = copy.deepcopy(
        next(case for case in cases if case["category"] == "long_context_handoff")
    )
    for assertion in broken["deterministic_assertions"]:
        if assertion["assertion_id"] == "assert.context.within_budget.v1":
            assertion["parameters"] = {"maximum_input_tokens": 0}
    findings = validator.validate_evaluation_cases([broken])
    assert "ASSERTION_PARAMETERS_INVALID" in {
        finding.code for finding in findings
    }
...
def test_handoff_assertion_rejects_duplicate_fields(
    validator: OfflineRepositoryValidator, cases: list[dict]
) -> None:
    broken = copy.deepcopy(
        next(case for case in cases if case["category"] == "long_context_handoff")
    ...
    for assertion in broken["deterministic_assertions"]:
        if assertion["assertion_id"] == "assert.handoff.fields_allowed.v1":
            assertion["parameters"] = {"allowed_fields": ["ticket_id", "ticket_id"]}
    findings = validator.validate_evaluation_cases([broken])
    assert "ASSERTION_PARAMETERS_INVALID" in {
        finding.code for finding in findings
    }
